...
handles.fname=input('enter the name of "mat" file\n','s');
handles.imnumber=input('input image number which u want to open\n');
cd ../data
load(handles.fname)
...
% Flip the image upside down before showing it
imagesc([-max(k(:)) max(k(:))], [-2.5 3.5], flipud(handles.img));
 colormap('gray');

hold on;
%plot(k,yaxis*.9+.5,'LineWidth',2,'Color',[0 1 0])
plot(k,back+.3,'LineWidth',2,'Color',[0 .5 1])
...
% Flip the image upside down before showing it
% Flip the image upside down before showing it
imagesc([-max(k(:)) max(k(:))], [-1.5 1.5], flipud(handles.img.^.05));
 colormap('gray');

hold on;
plot(k,yaxis,'LineWidth',2,'Color',[0 1 0])
plot(k,back,'LineWidth',2,'Color',[0 .5 1])
axis([-.8*10^10 0 0 1]);
